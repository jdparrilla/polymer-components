...
<html xml:lang="${cmsfn.language()}" lang="${cmsfn.language()}">
  <head>
    ...
    [#-- To load resources you can link them manually (e.g. line below) --]
      [#-- <link rel="stylesheet" type="text/css" href="${ctx.contextPath}/.resources/polymer-components-module/webresources/css/bootstrap.css" media="all" /> --]
      [#-- <script src="${ctx.contextPath}/.resources/polymer-components-module/webresources/js/jquery.js"></script> --]
    [#-- or via theme --]
    ...
    [#-- uncomment next line to use resfn templating functions to load all css which matches a globbing pattern --]
      [#-- ${resfn.css(["/polymer-components-module/.*css"])!} --]
  </head>
  <body class="polymer-page ${cmsfn.language()}">

    <div class="container ">
      <h1>polymer-page works!</h1>
      [@cms.area name="main"/]
    </div>

    [#-- use resfn to load all js which matches the globbing pattern or link resources manually or via theme --]
    [#-- ${resfn.js(["/polymer-components-module/.*js"])!} --]
  </body>
</html>